...
preview contract: The preview must re-fetch the source award row, tokenize visible labels, show current `nominee_ids`, show proposed IDs, show the entity kind, and show the exact final `nominee_ids` string before any apply action is available.
...
- every company slot is blank or a valid `co...` ID.
...
test: Contract tests prove schema creation, allowed states, entity-kind validation, rejection of `nm...` IDs, rejection of fake department IDs, exact confirmation, single-row mutation boundaries, reporting rebuild invocation, and public-marker non-leakage.
...
- Company and department work does not resume in bulk until the classifier gives us a trustworthy count and sample list.
