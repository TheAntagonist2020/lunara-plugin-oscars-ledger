...
- 2026-06-26 live checkpoint after plugin `2.7.57`: sampled `state=company` Sound Mixing rows already had ordered `co...` IDs (`10` sampled rows, `0` blank IDs, `0` slot mismatches), while sampled `state=department` rows were correctly blank/text-led (`43` sampled rows, `43` blank IDs, `0` slot mismatches). Do not implement or use a company apply path unless a true company row with missing or incorrect `co...` IDs is found.
