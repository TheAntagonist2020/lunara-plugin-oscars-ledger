...
= 2.7.89 =
* Added a private Fix First Review Pack to the Image Integrity Console so the first 25 wrong-match/unreviewed rows can be worked as a focused cleanup batch.
* Kept the review pack read-only and derived from existing normalized Image Integrity rows; no public routes, image imports, media deletes, external API calls, or Oscar result-row mutations changed.
...
* Kept the strip media-safe by using existing verified visual packages and rendering intentional text-led cards when no public image is available.

= 2.7.77 =
* Filled premium category era-browser poster chambers from verified nominee title visuals after winner visuals are exhausted.
* Added source labels on era-browser visuals so winner and nominee imagery stays public-facing and honest.

= 2.7.76 =
* Added category-depth scale hooks so brief, compact, standard, and marathon Oscar category dossiers size themselves intentionally.
* Resized premium category era-browser poster grids by verified visual count, giving richer decades larger visual lanes while keeping single-poster decades controlled.
* Tightened long-running category ledger density so large category histories feel more dynamic and less like static archival stretches.
...
= 2.7.72 =
* Promoted Assistant Director and Dance Direction into the premium Oscars category dossier system, giving the early-craft curiosity routes a stronger historical presentation without changing data or URLs.

= 2.7.71 =
* Promoted early Short Subject category routes for Comedy, Novelty, Color, One-reel, and Two-reel into the premium Oscars category dossier system, extending the short-form route family without changing data or URLs.
...
= 2.7.66 =
* Promoted Black-and-White and Color Cinematography split routes to the premium Oscars category dossier system, completing the historical cinematography route family without changing data or URLs.

= 2.7.65 =
* Promoted Documentary Short Subject to the premium Oscars category dossier system, extending the documentary route family without changing data or URLs.

= 2.7.64 =
* Promoted Live Action Short Film to the premium Oscars category dossier system, completing the shorts-route pair without changing data or URLs.
...
= 2.7.61 =
* Promoted Makeup and Hairstyling to the premium Oscars category dossier system, adding a stronger transformation-craft route rhythm without changing data or URLs.
...
* Full-row reviews store ordered nominee ID proposals in `wp_aat_person_credit_row_reviews` without mutating Oscar rows.
* Applying a full-row correction requires Ready To Apply state, exact visible-label/ID count matching, a checkbox, and typed source award row confirmation.
