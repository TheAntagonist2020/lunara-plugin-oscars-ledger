...
<div class="aat-container aat-hub-page">
    ...
    <?php
        ...
        elseif ($hub === 'ceremony') :
            ...
            $ceremony_race_highlight_cards = array();
            ...
            foreach ($ceremony_major_race_groups as $briefing_group) {
                ...
                $highlight_row = !empty($briefing_winner_row)
                    ? $briefing_winner_row
                    : (!empty($briefing_rows[0]) ? $aat_enrich_winner_entry_links($briefing_rows[0]) : array());
                $highlight_title_ids = $aat_extract_title_ids($highlight_row);
                $highlight_title_id = !empty($highlight_title_ids[0]) ? (string) $highlight_title_ids[0] : '';
                $highlight_visual = $highlight_title_id !== '' ? $aat_get_visual_package($highlight_title_id, 'medium_large') : array();

                if ($highlight_title_id !== '' && (!empty($highlight_visual['poster_html']) || !empty($highlight_visual['poster_url']))) {
                    $highlight_primary_label = trim((string) ($highlight_row['primary_label'] ?? (!empty($highlight_row) ? $aat_winner_primary($highlight_row) : '')));
                    $highlight_primary_url = !empty($highlight_row['primary_url']) ? (string) $highlight_row['primary_url'] : '';
                    $highlight_secondary_label = trim((string) ($highlight_row['secondary_label'] ?? (!empty($highlight_row) ? $aat_winner_secondary($highlight_row) : '')));

                    if ($highlight_primary_label !== '') {
                        $ceremony_race_highlight_cards[] = array(
                            'category'        => $briefing_category,
                            'category_key'    => $briefing_category_key,
                            'label'           => $briefing_label,
                            'category_url'    => $briefing_url,
                            'ledger_url'      => $ceremony_ballot_full_url . '#ceremony-category-' . sanitize_title($briefing_category),
                            'field_count'     => count($briefing_rows),
                            'title_id'        => $highlight_title_id,
                            'primary_label'   => $highlight_primary_label,
                            'primary_url'     => $highlight_primary_url,
                            'secondary_label' => $highlight_secondary_label,
                            'review_url'      => !empty($ceremony_review_map[$highlight_title_id]) ? (string) $ceremony_review_map[$highlight_title_id] : '',
                            'visual'          => $highlight_visual,
                            'is_complete'     => !empty($briefing_winner_rows),
                        );
                    }
                }
    ...
    ?>
        ...
        <?php if (!empty($ceremony_race_highlight_cards)) : ?>
            <section class="aat-hub-section aat-ceremony-race-highlights" aria-label="<?php echo esc_attr__('Ceremony race highlights', 'academy-awards-table'); ?>">
                <div class="aat-ceremony-race-highlights-head">
                    <div>
                        <p class="aat-hub-kicker"><?php echo esc_html__('Race Highlights', 'academy-awards-table'); ?></p>
                        <h2><?php echo esc_html__('The four-race visual briefing', 'academy-awards-table'); ?></h2>
                        <p class="aat-hub-copy"><?php echo esc_html__('Picture, Director, Actor, and Actress get a quick visual read before the ceremony file opens into the full ledger.', 'academy-awards-table'); ?></p>
                    </div>
                    <a class="aat-hub-card-action" href="<?php echo esc_url($ceremony_ballot_full_url); ?>"><?php echo esc_html__('Open Full Ballot', 'academy-awards-table'); ?></a>
                </div>
                <div class="aat-ceremony-race-strip" aria-label="<?php echo esc_attr__('Major race visual cards', 'academy-awards-table'); ?>">
                    <?php foreach ($ceremony_race_highlight_cards as $race_highlight_card) :
                        $race_visual = !empty($race_highlight_card['visual']) && is_array($race_highlight_card['visual']) ? $race_highlight_card['visual'] : array();
                        $race_backdrop_style = $aat_get_card_backdrop_style($race_visual['poster_url'] ?? '', $race_visual['backdrop_url'] ?? '');
                        $race_title_url = !empty($race_highlight_card['primary_url']) ? (string) $race_highlight_card['primary_url'] : (string) $race_highlight_card['ledger_url'];
                        $race_card_classes = array(
                            'aat-ceremony-race-card',
                            'is-major-race',
                            !empty($race_highlight_card['is_complete']) ? 'is-complete' : 'is-field',
                            'is-' . sanitize_html_class(strtolower(str_replace(' ', '-', (string) $race_highlight_card['category_key']))),
                        );
                        if (strtoupper((string) $race_highlight_card['category_key']) === 'BEST PICTURE') {
                            $race_card_classes[] = 'is-best-picture';
                        }
                        if ($race_backdrop_style !== '') {
                            $race_card_classes[] = 'aat-card-has-backdrop';
                        }
                    ?>
                        <article class="<?php echo esc_attr(implode(' ', array_filter($race_card_classes))); ?>"<?php echo $race_backdrop_style !== '' ? ' style="' . esc_attr($race_backdrop_style) . '"' : ''; ?>>
                            <a class="aat-ceremony-race-media" href="<?php echo esc_url($race_title_url); ?>">
                                <?php if (!empty($race_visual['poster_html'])) : ?>
                                    <?php echo $race_visual['poster_html']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                                <?php else : ?>
                                    <img class="aat-ceremony-race-poster" src="<?php echo esc_url($race_visual['poster_url']); ?>" alt="<?php echo esc_attr(sprintf(__('%s poster', 'academy-awards-table'), (string) $race_highlight_card['primary_label'])); ?>" loading="lazy" decoding="async" />
                                <?php endif; ?>
                            </a>
                            <div class="aat-ceremony-race-body">
                                <div class="aat-ceremony-race-meta">
                                    <?php echo $aat_render_hub_text_link((string) $race_highlight_card['label'], (string) $race_highlight_card['category_url'], 'aat-ceremony-race-category aat-hub-inline-link'); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                                    <span><?php echo esc_html(!empty($race_highlight_card['is_complete']) ? __('Winner', 'academy-awards-table') : __('Field', 'academy-awards-table')); ?></span>
                                </div>
                                <h3>
                                    <?php echo $aat_render_hub_text_link((string) $race_highlight_card['primary_label'], $race_title_url, 'aat-hub-inline-link aat-hub-inline-link-title'); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                                </h3>
                                <?php if (!empty($race_highlight_card['secondary_label'])) : ?>
                                    <p><?php echo esc_html($race_highlight_card['secondary_label']); ?></p>
                                <?php endif; ?>
                                <div class="aat-ceremony-race-actions">
                                    <a class="aat-hub-card-action" href="<?php echo esc_url($race_highlight_card['ledger_url']); ?>"><?php echo esc_html__('Race Ledger', 'academy-awards-table'); ?></a>
                                    <?php if (!empty($race_highlight_card['review_url'])) : ?>
                                        <a class="aat-hub-card-action is-review" href="<?php echo esc_url($race_highlight_card['review_url']); ?>"><?php echo esc_html__('Lunara Review', 'academy-awards-table'); ?></a>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </article>
        ...
        <?php if (!empty($ceremony_ballot_groups)) : ?>
            ...
            ?>
            <section class="aat-hub-section aat-ceremony-ballot-ledger<?php echo $ceremony_ballot_full_requested ? ' is-full-ledger' : ' is-fast-ledger'; ?>">
                <div class="aat-section-head">
                ...
                <div class="aat-ceremony-ballot-summary">
                    ...
                    <?php else : ?>
                        <span><?php echo esc_html__('Every nominee expanded', 'academy-awards-table'); ?></span>
                    <?php endif; ?>
                </div>
                <?php if ($ceremony_ballot_full_requested) : ?>
                    <div class="aat-ceremony-full-ledger-brief">
                        <div>
                            <p class="aat-hub-kicker"><?php echo esc_html__('Research View', 'academy-awards-table'); ?></p>
                            <strong><?php echo esc_html__('Every ballot trail is open.', 'academy-awards-table'); ?></strong>
                            <span><?php echo esc_html__('The ceremony stays chaptered by category while nominee, title, person, review, and history links remain directly reachable.', 'academy-awards-table'); ?></span>
                        </div>
                        <div class="aat-ceremony-full-ledger-stats" aria-label="<?php echo esc_attr__('Expanded ceremony ledger stats', 'academy-awards-table'); ?>">
                            <span class="aat-ceremony-full-ledger-stat"><strong><?php echo esc_html(number_format_i18n($noms)); ?></strong><em><?php echo esc_html__('records', 'academy-awards-table'); ?></em></span>
                            <span class="aat-ceremony-full-ledger-stat"><strong><?php echo esc_html(number_format_i18n(max(0, $noms - $wins))); ?></strong><em><?php echo esc_html__('nominee rows', 'academy-awards-table'); ?></em></span>
                            <span class="aat-ceremony-full-ledger-stat"><strong><?php echo esc_html(number_format_i18n(count($ceremony_ballot_groups))); ?></strong><em><?php echo esc_html__('chapters', 'academy-awards-table'); ?></em></span>
                        </div>
                    </div>
                <?php endif; ?>
                                ...
                                ?>
                                    <article class="aat-ceremony-ballot-row<?php echo $is_row_winner ? ' is-winner' : ''; ?>">
                                        <div class="aat-ceremony-ballot-status">
                                            <?php if ($ceremony_ballot_full_requested) : ?>
                                                <span class="aat-ceremony-ballot-row-index"><?php echo esc_html(sprintf('%02d', $ballot_row_index)); ?></span>
                                            <?php endif; ?>
                                            ...
                                            <?php if ($ceremony_ballot_full_requested) : ?>
                                                <span class="aat-ceremony-ballot-depth"><?php echo esc_html(sprintf(__('%1$s of %2$s', 'academy-awards-table'), number_format_i18n($ballot_row_index), number_format_i18n(count($rendered_group_rows)))); ?></span>
                                            <?php endif; ?>
                                        </div>
    ...
    <?php
        ...
        elseif ($hub === 'category') :
            ...
            $premium_category_profiles = array(
                ...
                'SPECIAL AWARD' => array(
                    'class' => 'aat-archive-specials-dossier aat-special-award-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Special Award Dossier', 'academy-awards-table'),
                    'subtitle' => __('The special-award file: one-off Academy recognitions, historic exceptions, and the preserved honors that should read as curated archive evidence instead of stray ledger rows.', 'academy-awards-table'),
                    'history_kicker' => __('Special Archive Browser', 'academy-awards-table'),
                    'history_title' => __('Special Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Special Award archive.', 'academy-awards-table'),
                ),
                'HONORARY AWARD' => array(
                    'class' => 'aat-archive-specials-dossier aat-honorary-award-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Honorary Award Dossier', 'academy-awards-table'),
                    'subtitle' => __('The honorary file: lifetime tributes, institutional memory, career coronations, and the Academy honors that sit beside the competitive record.', 'academy-awards-table'),
                    'history_kicker' => __('Honorary Archive Browser', 'academy-awards-table'),
                    'history_title' => __('Honorary Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Honorary Award archive.', 'academy-awards-table'),
                ),
                'SPECIAL ACHIEVEMENT AWARD' => array(
                    'class' => 'aat-archive-specials-dossier aat-special-achievement-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Special Achievement Award Dossier', 'academy-awards-table'),
                    'subtitle' => __('The achievement file: rare Academy recognitions for films whose craft breakthroughs demanded a lane outside the standard competitive map.', 'academy-awards-table'),
                    'history_kicker' => __('Achievement Archive Browser', 'academy-awards-table'),
                    'history_title' => __('Special Achievement Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Special Achievement Award archive.', 'academy-awards-table'),
                ),
                'SPECIAL ACHIEVEMENT AWARD (SOUND EDITING)' => array(
                    'class' => 'aat-archive-specials-dossier aat-special-achievement-sound-editing-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Special Achievement Award: Sound Editing Dossier', 'academy-awards-table'),
                    'subtitle' => __('A preserved sound-editing exception file for the Academy recognition that sits between the craft ledger and the special-achievement archive.', 'academy-awards-table'),
                    'history_kicker' => __('Sound Achievement Browser', 'academy-awards-table'),
                    'history_title' => __('Special Sound Editing Achievement Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Special Achievement Sound Editing archive.', 'academy-awards-table'),
                ),
                'SPECIAL ACHIEVEMENT AWARD (SOUND EFFECTS EDITING)' => array(
                    'class' => 'aat-archive-specials-dossier aat-special-achievement-sound-effects-editing-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Special Achievement Award: Sound Effects Editing Dossier', 'academy-awards-table'),
                    'subtitle' => __('The sound-effects editing exception file: a compact archive for the rare recognitions that predate or sit outside standard craft-category continuity.', 'academy-awards-table'),
                    'history_kicker' => __('Sound Effects Archive Browser', 'academy-awards-table'),
                    'history_title' => __('Special Sound Effects Editing Achievement Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Special Achievement Sound Effects Editing archive.', 'academy-awards-table'),
                ),
                'SPECIAL ACHIEVEMENT AWARD (SOUND EFFECTS)' => array(
                    'class' => 'aat-archive-specials-dossier aat-special-achievement-sound-effects-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Special Achievement Award: Sound Effects Dossier', 'academy-awards-table'),
                    'subtitle' => __('The sound-effects achievement file: a narrow archive lane for a rare Academy craft recognition outside the ordinary ballot structure.', 'academy-awards-table'),
                    'history_kicker' => __('Sound Effects Archive Browser', 'academy-awards-table'),
                    'history_title' => __('Special Sound Effects Achievement Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Special Achievement Sound Effects archive.', 'academy-awards-table'),
                ),
                'SPECIAL ACHIEVEMENT AWARD (VISUAL EFFECTS)' => array(
                    'class' => 'aat-archive-specials-dossier aat-special-achievement-visual-effects-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Special Achievement Award: Visual Effects Dossier', 'academy-awards-table'),
                    'subtitle' => __('The visual-effects achievement file: breakthrough spectacles and exceptional craft recognitions preserved outside the standard competitive effects lineage.', 'academy-awards-table'),
                    'history_kicker' => __('Effects Achievement Browser', 'academy-awards-table'),
                    'history_title' => __('Special Visual Effects Achievement Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Special Achievement Visual Effects archive.', 'academy-awards-table'),
                ),
                'SPECIAL FOREIGN LANGUAGE FILM AWARD' => array(
                    'class' => 'aat-archive-specials-dossier aat-special-foreign-language-film-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Special Foreign Language Film Award Dossier', 'academy-awards-table'),
                    'subtitle' => __('The pre-category international file: special foreign-language recognitions that belong in the archive before the competitive international feature structure arrives.', 'academy-awards-table'),
                    'history_kicker' => __('International Archive Browser', 'academy-awards-table'),
                    'history_title' => __('Special Foreign Language Film Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Special Foreign Language Film Award archive.', 'academy-awards-table'),
                ),
                'IRVING G. THALBERG MEMORIAL AWARD' => array(
                    'class' => 'aat-archive-specials-dossier aat-thalberg-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Irving G. Thalberg Memorial Award Dossier', 'academy-awards-table'),
                    'subtitle' => __('The producer-honor file: the Academy archive for sustained creative production, studio authorship, and the industry builders honored outside a single film race.', 'academy-awards-table'),
                    'history_kicker' => __('Producer Honor Browser', 'academy-awards-table'),
                    'history_title' => __('Thalberg Memorial Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Irving G. Thalberg Memorial Award archive.', 'academy-awards-table'),
                ),
                'JEAN HERSHOLT HUMANITARIAN AWARD' => array(
                    'class' => 'aat-archive-specials-dossier aat-hersholt-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Jean Hersholt Humanitarian Award Dossier', 'academy-awards-table'),
                    'subtitle' => __('The humanitarian file: service, industry citizenship, and Academy memory beyond the competitive ballot.', 'academy-awards-table'),
                    'history_kicker' => __('Humanitarian Archive Browser', 'academy-awards-table'),
                    'history_title' => __('Jean Hersholt Humanitarian Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Jean Hersholt Humanitarian Award archive.', 'academy-awards-table'),
                ),
                'GORDON E. SAWYER AWARD' => array(
                    'class' => 'aat-archive-specials-dossier aat-sawyer-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Gordon E. Sawyer Award Dossier', 'academy-awards-table'),
                    'subtitle' => __('The technical-honor file: sustained engineering contribution, cinema machinery, and the people whose inventions shaped how films are made.', 'academy-awards-table'),
                    'history_kicker' => __('Technical Honor Browser', 'academy-awards-table'),
                    'history_title' => __('Gordon E. Sawyer Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Gordon E. Sawyer Award archive.', 'academy-awards-table'),
                ),
                'JOHN A. BONNER MEDAL OF COMMENDATION' => array(
                    'class' => 'aat-archive-specials-dossier aat-bonner-medal-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('John A. Bonner Medal of Commendation Dossier', 'academy-awards-table'),
                    'subtitle' => __('The commendation file: Academy technical service, standards, and behind-the-scenes contributions that deserve a real archive lane.', 'academy-awards-table'),
                    'history_kicker' => __('Commendation Browser', 'academy-awards-table'),
                    'history_title' => __('John A. Bonner Medals Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s John A. Bonner Medal of Commendation archive.', 'academy-awards-table'),
                ),
                'MEDAL OF COMMENDATION' => array(
                    'class' => 'aat-archive-specials-dossier aat-medal-commendation-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Medal of Commendation Dossier', 'academy-awards-table'),
                    'subtitle' => __('The medal file: technical and institutional commendations preserved as an archive of Academy craft stewardship.', 'academy-awards-table'),
                    'history_kicker' => __('Medal Archive Browser', 'academy-awards-table'),
                    'history_title' => __('Medals of Commendation Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Medal of Commendation archive.', 'academy-awards-table'),
                ),
                'AWARD OF COMMENDATION' => array(
                    'class' => 'aat-archive-specials-dossier aat-award-commendation-dossier',
                    'kicker' => __('Oscar Archive Special', 'academy-awards-table'),
                    'title' => __('Award of Commendation Dossier', 'academy-awards-table'),
                    'subtitle' => __('The award-of-commendation file: institutional craft recognition and Academy gratitude kept visible instead of buried in ledger miscellany.', 'academy-awards-table'),
                    'history_kicker' => __('Commendation Archive Browser', 'academy-awards-table'),
                    'history_title' => __('Awards of Commendation Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Award of Commendation archive.', 'academy-awards-table'),
                ),
                'SCIENTIFIC OR TECHNICAL AWARD (CLASS I)' => array(
                    'class' => 'aat-archive-specials-dossier aat-scitech-dossier aat-scitech-class-i-dossier',
                    'kicker' => __('Oscar SciTech Archive', 'academy-awards-table'),
                    'title' => __('Scientific or Technical Award: Class I Dossier', 'academy-awards-table'),
                    'subtitle' => __('The Class I SciTech file: major technical contributions from the early classification system, preserved as part of the Academy machinery record.', 'academy-awards-table'),
                    'history_kicker' => __('SciTech Class Browser', 'academy-awards-table'),
                    'history_title' => __('Scientific or Technical Class I Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Scientific or Technical Class I archive.', 'academy-awards-table'),
                ),
                'SCIENTIFIC OR TECHNICAL AWARD (CLASS II)' => array(
                    'class' => 'aat-archive-specials-dossier aat-scitech-dossier aat-scitech-class-ii-dossier',
                    'kicker' => __('Oscar SciTech Archive', 'academy-awards-table'),
                    'title' => __('Scientific or Technical Award: Class II Dossier', 'academy-awards-table'),
                    'subtitle' => __('The Class II SciTech file: engineering recognitions from the Academy archive, connecting inventions, departments, and craft infrastructure.', 'academy-awards-table'),
                    'history_kicker' => __('SciTech Class Browser', 'academy-awards-table'),
                    'history_title' => __('Scientific or Technical Class II Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Scientific or Technical Class II archive.', 'academy-awards-table'),
                ),
                'SCIENTIFIC OR TECHNICAL AWARD (CLASS III)' => array(
                    'class' => 'aat-archive-specials-dossier aat-scitech-dossier aat-scitech-class-iii-dossier',
                    'kicker' => __('Oscar SciTech Archive', 'academy-awards-table'),
                    'title' => __('Scientific or Technical Award: Class III Dossier', 'academy-awards-table'),
                    'subtitle' => __('The Class III SciTech file: technical achievement recognition across the early Academy system, kept readable as craft history rather than administrative residue.', 'academy-awards-table'),
                    'history_kicker' => __('SciTech Class Browser', 'academy-awards-table'),
                    'history_title' => __('Scientific or Technical Class III Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Scientific or Technical Class III archive.', 'academy-awards-table'),
                ),
                'SCIENTIFIC AND TECHNICAL AWARD (ACADEMY AWARD OF MERIT)' => array(
                    'class' => 'aat-archive-specials-dossier aat-scitech-dossier aat-scitech-merit-dossier',
                    'kicker' => __('Oscar SciTech Archive', 'academy-awards-table'),
                    'title' => __('Scientific and Technical Award: Academy Award of Merit Dossier', 'academy-awards-table'),
                    'subtitle' => __('The Academy Award of Merit file: the top SciTech honor, tracing major engineering achievements and the tools that changed cinema practice.', 'academy-awards-table'),
                    'history_kicker' => __('SciTech Merit Browser', 'academy-awards-table'),
                    'history_title' => __('Academy Award of Merit Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Academy Award of Merit archive.', 'academy-awards-table'),
                ),
                'SCIENTIFIC AND TECHNICAL AWARD (SCIENTIFIC AND ENGINEERING AWARD)' => array(
                    'class' => 'aat-archive-specials-dossier aat-scitech-dossier aat-scitech-engineering-dossier',
                    'kicker' => __('Oscar SciTech Archive', 'academy-awards-table'),
                    'title' => __('Scientific and Technical Award: Scientific and Engineering Award Dossier', 'academy-awards-table'),
                    'subtitle' => __('The Scientific and Engineering file: cinema tools, processes, systems, and the technical teams whose work belongs in the visible Academy record.', 'academy-awards-table'),
                    'history_kicker' => __('SciTech Engineering Browser', 'academy-awards-table'),
                    'history_title' => __('Scientific and Engineering Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Scientific and Engineering Award archive.', 'academy-awards-table'),
                ),
                'SCIENTIFIC AND TECHNICAL AWARD (TECHNICAL ACHIEVEMENT AWARD)' => array(
                    'class' => 'aat-archive-specials-dossier aat-scitech-dossier aat-scitech-technical-achievement-dossier',
                    'kicker' => __('Oscar SciTech Archive', 'academy-awards-table'),
                    'title' => __('Scientific and Technical Award: Technical Achievement Award Dossier', 'academy-awards-table'),
                    'subtitle' => __('The Technical Achievement file: the dense engineering ledger behind production, exhibition, sound, image, preservation, and every practical tool that moved cinema forward.', 'academy-awards-table'),
                    'history_kicker' => __('SciTech Technical Browser', 'academy-awards-table'),
                    'history_title' => __('Technical Achievement Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Technical Achievement Award archive.', 'academy-awards-table'),
                ),
                'SCIENTIFIC AND TECHNICAL AWARD (SPECIAL AWARD)' => array(
                    'class' => 'aat-archive-specials-dossier aat-scitech-dossier aat-scitech-special-award-dossier',
                    'kicker' => __('Oscar SciTech Archive', 'academy-awards-table'),
                    'title' => __('Scientific and Technical Award: Special Award Dossier', 'academy-awards-table'),
                    'subtitle' => __('A preserved SciTech exception file for the rare special technical recognition that belongs beside the formal Academy engineering classes.', 'academy-awards-table'),
                    'history_kicker' => __('SciTech Special Browser', 'academy-awards-table'),
                    'history_title' => __('Scientific and Technical Special Awards Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Scientific and Technical Special Award archive.', 'academy-awards-table'),
                ),
                ...
                'SHORT SUBJECT (COMEDY)' => array(
                    'class' => 'aat-short-film-dossier aat-short-subject-dossier aat-short-subject-comedy-dossier',
                    'title' => __('Comedy Short Subject Dossier', 'academy-awards-table'),
                    'subtitle' => __('The comedy short file: studio gag machinery, specialty producers, compact comic premises, and the early Oscar branch that tracked laughter before the modern shorts map settled.', 'academy-awards-table'),
                    'history_kicker' => __('Comedy Shorts Era Browser', 'academy-awards-table'),
                    'history_title' => __('Comedy Short Subject Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Comedy Short Subject run.', 'academy-awards-table'),
                ),
                'SHORT SUBJECT (NOVELTY)' => array(
                    'class' => 'aat-short-film-dossier aat-short-subject-dossier aat-short-subject-novelty-dossier',
                    'title' => __('Novelty Short Subject Dossier', 'academy-awards-table'),
                    'subtitle' => __('The novelty short file: travelogues, technical curiosities, specialty reels, and the brief-format experiments that made early Oscar history stranger and more textured.', 'academy-awards-table'),
                    'history_kicker' => __('Novelty Shorts Era Browser', 'academy-awards-table'),
                    'history_title' => __('Novelty Short Subject Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Novelty Short Subject run.', 'academy-awards-table'),
                ),
                'SHORT SUBJECT (COLOR)' => array(
                    'class' => 'aat-short-film-dossier aat-short-subject-dossier aat-short-subject-color-dossier',
                    'title' => __('Color Short Subject Dossier', 'academy-awards-table'),
                    'subtitle' => __('The color short file: early Technicolor spectacle, promotional craft, studio demonstrations, and the compact films that let Oscar history test color in concentrated form.', 'academy-awards-table'),
                    'history_kicker' => __('Color Shorts Era Browser', 'academy-awards-table'),
                    'history_title' => __('Color Short Subject Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Color Short Subject run.', 'academy-awards-table'),
                ),
                'SHORT SUBJECT (ONE-REEL)' => array(
                    'class' => 'aat-short-film-dossier aat-short-subject-dossier aat-short-subject-one-reel-dossier',
                    'title' => __('One-reel Short Subject Dossier', 'academy-awards-table'),
                    'subtitle' => __('The one-reel short file: tight studio units, compressed nonfiction and comedy forms, and the fast-running Oscar branch where a single reel had to make the whole case.', 'academy-awards-table'),
                    'history_kicker' => __('One-reel Shorts Era Browser', 'academy-awards-table'),
                    'history_title' => __('One-reel Short Subject Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s One-reel Short Subject run.', 'academy-awards-table'),
                ),
                'SHORT SUBJECT (TWO-REEL)' => array(
                    'class' => 'aat-short-film-dossier aat-short-subject-dossier aat-short-subject-two-reel-dossier',
                    'title' => __('Two-reel Short Subject Dossier', 'academy-awards-table'),
                    'subtitle' => __('The two-reel short file: expanded short-form storytelling, studio specialty units, and the early Academy trail where compact films got just enough room to breathe.', 'academy-awards-table'),
                    'history_kicker' => __('Two-reel Shorts Era Browser', 'academy-awards-table'),
                    'history_title' => __('Two-reel Short Subject Through the Eras', 'academy-awards-table'),
                    'era_copy' => __('A visual entry point into the %1$s Two-reel Short Subject run.', 'academy-awards-table'),
                ),
            ...
            $category_scale_class = 'aat-category-scale-standard';
            if ($is_premium_category_dossier) {
                if ($cers <= 5) {
                    $category_scale_class = 'aat-category-scale-brief';
                } elseif ($cers <= 25) {
                    $category_scale_class = 'aat-category-scale-compact';
                } elseif ($cers >= 75) {
                    $category_scale_class = 'aat-category-scale-marathon';
                }
            }
    ?>
        <div class="aat-category-dossier aat-inner-route-system<?php echo $is_premium_category_dossier ? ' aat-premium-category-dossier ' . esc_attr($category_scale_class) . ' ' . esc_attr((string) $premium_category_profile['class']) : ' aat-generic-category-dossier'; ?>">
        <?php if ($is_premium_category_dossier) : ?>
            <style>
                ...
                body .aat-container .aat-premium-category-dossier.aat-archive-specials-dossier .aat-dossier-hero .aat-hub-title{font-size:clamp(2.15rem,4vw,4.1rem)!important;line-height:1.02!important;max-width:min(100%,14.5ch)!important;overflow-wrap:normal!important;word-break:normal!important;hyphens:none!important;text-wrap:balance!important}
                body .aat-container .aat-premium-category-dossier.aat-scitech-dossier .aat-dossier-hero .aat-hub-title{font-size:clamp(2rem,3.4vw,3.5rem)!important;max-width:min(100%,18ch)!important}
                ...
                body .aat-container .aat-premium-category-dossier.aat-category-scale-brief{gap:clamp(16px,2.2vw,26px)!important}
                body .aat-container .aat-premium-category-dossier.aat-category-scale-brief .aat-dossier-hero{padding:clamp(20px,3vw,34px)!important}
                body .aat-container .aat-premium-category-dossier.aat-category-scale-brief .aat-dossier-command-card{min-height:96px!important}
                body .aat-container .aat-premium-category-dossier.aat-category-scale-brief .aat-dossier-command-card.is-latest{min-height:124px!important}
                body .aat-container .aat-premium-category-dossier.aat-category-scale-marathon{gap:clamp(18px,2.4vw,28px)!important}
                body .aat-container .aat-premium-category-dossier.aat-category-scale-marathon .aat-era-chapter{margin-top:clamp(18px,3vw,34px)!important;padding:clamp(14px,2vw,22px)!important}
                body .aat-container .aat-premium-category-dossier.aat-category-scale-marathon .aat-decade-ceremonies{gap:10px!important}
                body .aat-container .aat-premium-category-dossier.aat-category-scale-marathon .aat-ledger-card{grid-template-columns:minmax(96px,.2fr) minmax(0,1fr)!important;gap:12px!important;padding:13px!important}
                body .aat-container .aat-premium-category-dossier.aat-category-scale-marathon .aat-category-history-title{font-size:clamp(1.05rem,1.3vw,1.32rem)!important}
                body .aat-container .aat-premium-category-dossier .aat-era-chapter-visual{grid-template-columns:minmax(220px,min(46vw,520px)) minmax(0,1fr)!important}
                body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid{width:100%!important;justify-self:start!important;max-width:520px!important}
                body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid.is-count-1{grid-template-columns:minmax(0,190px)!important;max-width:190px!important}
                body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid.is-count-2{grid-template-columns:repeat(2,minmax(0,1fr))!important;max-width:340px!important}
                body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid.is-count-3{grid-template-columns:repeat(3,minmax(0,1fr))!important;max-width:420px!important}
                body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid.is-count-4{grid-template-columns:repeat(4,minmax(0,1fr))!important;max-width:520px!important}
                @media(max-width:760px){body .aat-container .aat-premium-category-dossier .aat-era-chapter-visual{grid-template-columns:minmax(0,1fr)!important}body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid,body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid.is-count-2,body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid.is-count-3,body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid.is-count-4{max-width:100%!important}body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid.is-count-1{max-width:190px!important}}
                @media(max-width:620px){body .aat-container .aat-premium-category-dossier.aat-category-scale-marathon .aat-ledger-card,body .aat-container .aat-premium-category-dossier.aat-category-scale-brief .aat-ledger-card{grid-template-columns:minmax(0,1fr)!important}body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid.is-count-3{grid-template-columns:repeat(3,minmax(0,1fr))!important}body .aat-container .aat-premium-category-dossier .aat-era-chapter-media-grid.is-count-4{grid-template-columns:repeat(4,minmax(0,1fr))!important}}
            </style>
        <?php endif; ?>
        <?php if ($is_premium_category_dossier) : ?>
                    ...
                    <?php foreach ($category_decade_buckets as $decade_key => $decade_bucket) : ?>
                        <section class="aat-decade-group<?php echo $is_premium_category_dossier ? ' aat-era-chapter' : ''; ?>" id="aat-decade-<?php echo esc_attr($decade_key); ?>">
                            ...
                            <?php if (!empty($era_spotlights)) :
                                $era_visual_count = count($era_spotlights);
